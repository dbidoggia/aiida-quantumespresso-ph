...
def sssp(aiida_profile, generate_upf_data):  # pylint: disable=too-many-locals
...
def generate_calc_job_node(aiida_localhost):
    ...
    def _generate_calc_job_node( # pylint: disable=too-many-locals
...
def generate_inputs_ph(
    ...
    def _generate_inputs_ph(inputs=None):
        """Generate default inputs for a `PhCalculation."""
        from aiida.orm import Dict
        from aiida_quantumespresso.utils.resources import get_default_options

        parent_inputs = generate_inputs_pw()
...
@pytest.fixture
def generate_inputs_dynamical_matrix(generate_inputs_pw, generate_inputs_ph, generate_structure):
    """Generate default inputs for a `DynamicalMatrixWorkChain."""

    def _generate_inputs_dynamical_matrix():
        """Generate default inputs for a `DynamicalMatrixWorkChain."""
    ...
    return _generate_inputs_dynamical_matrix
